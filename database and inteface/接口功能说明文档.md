...
|获取某明星的相关帖子|star/{star_id}/posts/user/{user_id}|GET|-|{"state":"success",data:[{"post":post,"is_like":true},{"post":post,"is_like":false}]}|user_id用来判断用户是否喜欢某帖子|
|帖子详情|post/{post_id}/user/{user_id}|GET|-||{"state":"success",data:{post_detail:{"post":post,"is_like":true},comment_detail:[{"post":post,"is_like":false},{"post":post,"is_like":true}]}}|-|
|发贴|post|POST|user_id、title、content、star_id、imgs|{"state":"success","data":{"post_id":XX}}|-|
|发评论|post/{post_id}|POST|post_id、user_id、title、content、star_id、imgs|同上|-|
