...
func ImageID2Path(imageid string,suffix string)string{
	return fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s/%s/%s.%s",conf.Storage,imageid[0:2],imageid[2:4],imageid[4:6],imageid[6:8],imageid[8:10],imageid[10:12],imageid[12:14],imageid[14:16],suffix)
}
